...
bool SAVEGAME(){
   ...
   fprintf(SaveFileOpen, FORMAT_INDEX_OUT,index);
   for (int i = 0; i < FLAGS_NUMBER; i++)
   {
    fprintf(SaveFileOpen, FORMAT_FLAGS_OUT, FlagList[i].value);
   }
   for (int i = 0; i < CHARACTER_NUMBER; i++)
   {
    fprintf(SaveFileOpen, FORMAT_CHARA_OUT, CharaList[i].visible, CharaList[i].x, CharaList[i].y, CharaList[i].zindex, CharaList[i].expression_index);

    CharaList[i].gotox = CharaList[i].x;
    CharaList[i].gotoy = CharaList[i].y;
   }
   fprintf(SaveFileOpen, FORMAT_BG_OUT, Background.bg_index);
   fprintf(SaveFileOpen, FORMAT_MUSIC_OUT, MusicList.music_playing, MusicList.isplaying);
...
bool LOADGAME(){
    ...
    { //If file exist
        fscanf(SaveFileOpen,FORMAT_INDEX_IN, &index);
        for (int i = 0; i < FLAGS_NUMBER; i++)
        {
            fscanf(SaveFileOpen, FORMAT_FLAGS_IN, &FlagList[i].value);
        }
        for (int i = 0; i < CHARACTER_NUMBER; i++)
        {
            fscanf(SaveFileOpen, FORMAT_CHARA_IN, &CharaList[i].visible, &CharaList[i].x, &CharaList[i].y, &CharaList[i].zindex, &CharaList[i].expression_index);
            CharaList[i].gotox = CharaList[i].x;
            CharaList[i].gotoy = CharaList[i].y;
        }
        fscanf(SaveFileOpen, FORMAT_BG_IN, &Background.bg_index);
        fscanf(SaveFileOpen,FORMAT_MUSIC_IN, &MusicList.music_playing, &MusicList.isplaying);
...
bool SAVECONFIG(){
   ...
   fprintf(SaveFileOpen, FORMAT_OPTION_OUT, OPTION.volume, OPTION.check, OPTION.cps);
...
bool LOADCONFIG(){
    ...
    if (SaveFileOpen!=NULL)
    { //If file exist
        fscanf(SaveFileOpen, FORMAT_OPTION_IN, &OPTION.volume, &OPTION.check, &OPTION.cps);
    }
    else{
        playSomeSound();
    }
