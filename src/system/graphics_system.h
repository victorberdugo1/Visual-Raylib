...
void VN_DrawText(const char *text, int posX, int posY, float fontSize, Color color){
    ...
    for (int i = 0; i < length;)
    {
        ...
        else if (codepoint == '_' && GetCodepoint(&text[i+1], &codepointByteCount) == '_') // -> __
        {
        ...
        else if (codepoint == '[') 
        {
            ...
            {
                if (GetCodepoint(&text[i_bis], &codepointByteCount) == ']')
                {
        ...
        else
        {
            if ((codepoint != ' ') && (codepoint != '\t'))
            {
                ...
                if (flag_wave)
                {
                    position_x += sin(currentTime*wave_x_speed-i*wave_x_offset)*wave_x_range;
                    position_y += sin(currentTime*wave_y_speed-i*wave_y_offset)*wave_y_range;
                }
